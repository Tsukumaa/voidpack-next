            ...
            <div>
              ...
              <div className="flex gap-3 mt-2">
                {['Tous les dos de carte débloqués', '3 boosters VOID offerts par mois', 'Badge abonné sur le profil'].map(b => (
                  <span key={b} className="text-white/60 text-[10px] flex items-center gap-1">✓ {b}</span>
                ))}
              </div>
            </div>
            {!isSubscriber && (
            ...
            return (
              <div key={skin.id} className="rounded-2xl overflow-hidden border"
                ...
                <div className="aspect-[0.714] relative cursor-pointer"
                  ...
                  {isSelected && (
                    <div className="absolute top-2 left-2 w-5 h-5 rounded-full bg-[#7b2bff] flex items-center justify-center text-[10px] text-white">✓</div>
                  )}
                  {!accessible && (

...
export function CardHover({ rarity, children, className = '', style = {} }: CardHoverProps) {
  ...
  useEffect(() => {
    ...
    function frame() {
      t += .006
      const a    = .20 + .15 * Math.sin(t)
      const size = 120 + 18 * Math.sin(t * .65)
      el!.style.cssText = `
        position:absolute; border-radius:50%; filter:blur(30px); pointer-events:none;
        z-index:0; left:50%; top:50%;
        width:${size}%; height:${size}%;
        transform:translate(-50%,-50%);
        background:radial-gradient(circle,rgba(${glowRgb},${a}) 0%,rgba(${glowRgb},${a*.35}) 45%,transparent 70%);
      `
  ...
  useEffect(() => {
    ...
    let t = 0, raf = 0
    function draw(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, a: number) {
    ...
    function frame() {
      ...
      for (const s of stars) {
        ...
        draw(ctx, s.x, s.y, s.r, (.35 + .55*Math.sin(t*s.speed+s.phase)))
      }
      raf = requestAnimationFrame(frame)
    }
    raf = requestAnimationFrame(frame)
  ...
  const onMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    tiltRafRef.current = requestAnimationFrame(() => {
      ...
      const rx=(y-.5)*maxTilt, ry=-(x-.5)*maxTilt
  ...
  const onLeave = useCallback(() => {
    ...
    setTimeout(() => {
      if (card) { card.style.transition=''; card.style.animationPlayState='running' }
    }, 500)
  },[isVoid])

  return (
    // Wrapper — sert uniquement à positionner le glow derrière sans clip
    <div className={className} style={{ ...style, position:'relative' }}>

      {/* Glow derrière — void, en dehors du clip de la carte */}
      {isVoid && <div ref={glowBehindRef} />}

      {/* Carte — tilt + contenu + overlays */}
      <div ref={cardRef}

  ...
  return (
    <div className="pb-4 min-h-screen" style={{ background: '#06010e', color: '#f6f1ff' }}>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-8 gap-x-2.5 gap-y-12 px-3 pb-8">
          {filtered.map(card => {
                ...
                <CardHover rarity={card.rarity}
                  ...
                  {card.effects && card.effects.length > 0 && (
                    <div className="absolute left-0 right-0 flex flex-wrap justify-center gap-0.5 px-1 pointer-events-none" style={{ top: '9%', zIndex: 40 }}>
                      {card.effects.includes('taunt')      && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(200,80,0,.25)', color: '#ffb580', lineHeight: 1.3, border: '1px solid rgba(200,80,0,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>Provocation</span>}
                      {card.effects.includes('shield')     && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(0,110,220,.25)', color: '#7ec4ff', lineHeight: 1.3, border: '1px solid rgba(0,110,220,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>Bouclier</span>}
                      {card.effects.includes('charge')     && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(40,160,40,.25)', color: '#7edc7e', lineHeight: 1.3, border: '1px solid rgba(40,160,40,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>Charge</span>}
                      {card.effects.includes('lifesteal')  && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(180,20,60,.25)', color: '#ff7090', lineHeight: 1.3, border: '1px solid rgba(180,20,60,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>Vol de vie</span>}
                      {card.effects.includes('void_surge') && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(100,0,220,.25)', color: '#c070ff', lineHeight: 1.3, border: '1px solid rgba(100,0,220,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>VOID Surge</span>}
                      {card.effects.includes('stealth')    && <span style={{ fontSize: '8px', fontWeight: 600, padding: '2px 5px', borderRadius: 4, background: 'rgba(100,100,140,.25)', color: '#c0c0e0', lineHeight: 1.3, border: '1px solid rgba(100,100,140,.9)', boxShadow: '0 1px 4px rgba(0,0,0,.6)' }}>Furtivité</span>}
                    </div>
                  )}
                  {qty > 0 && (
